...
void simulateRandomLogic(string vecName, int num_vectors){
    ...
    for(int i =0; i < num_vectors; i++){
    	 for(int j = 0; j < circuit->numpri; j++){
            ...
            probTo0 = *(probMaskTo0 + j);
            probTo1 = *(probMaskTo1 + j);
            // only apply probTo0/1 when they are greater than 0
            if (probTo0 > 0 || probTo1 > 0) {
                val = (double)rand()/ RAND_MAX; //a rand num between 0 and 1
                if (probTo0 > probTo1){
                    if (val <= probTo0){
                        a = 0;
                    }
			...
			vecFile_w << a;				//COMMENT TO REMOVE RSM
...
void partition(int num_vectors, string vecName){
	...
	for(int i = 0; i < circuit->numff; i++){
		if(count_one_ffs[i] > count_zero_ffs[i]){
			ff_bias.push_back((count_one_ffs[i] - count_zero_ffs[i])/2000);//(count_one_ffs[i] + count_zero_ffs[i]));
		}else{
			ff_bias.push_back((count_zero_ffs[i] - count_one_ffs[i])/2000);//(count_one_ffs[i] + count_zero_ffs[i]));
		}
...
int main(int argc, char *argv[])
{
    ...
    for( int times =0; times < 40; times++){
		...
		for( int gen_num = 0; gen_num < 500 ; gen_num++){
            cout << "TIME#" << times << " GENERATION NUMBER "<< gen_num +1 << "\n";
